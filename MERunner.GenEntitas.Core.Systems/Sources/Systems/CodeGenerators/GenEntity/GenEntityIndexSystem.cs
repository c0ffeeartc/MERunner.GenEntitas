...
public sealed class Factory_GenEntityIndexSystem : TSystem_Factory<GenEntityIndexSystem> {  }
...
";
		...
		void generateEntityIndices(EntityIndexData[] data) {
			...
			var fileContent = CLASS_TEMPLATE
				...
				fileEnt.Add_( new GeneratedFileComp( "Contexts.cs", fileContent.WrapInNamespace( _contexts ), GetType().FullName ) );
		}
		...
		string generateMethods(EntityIndexData data, string contextName) {
			return ADD_INDEX_TEMPLATE
				...
				.Replace("${componentName}", data.GetComponentType().ToComponentName(_contexts.Get<Settings>().Is<IgnoreNamespaces>()).LowercaseFirst());
		}
